...
import { Process, BadgeIcon, Cancel } from '../../assets';
...
class ScanScreen extends Component {
        ...
        verifyHash(this.state.dlHashOne)
            .then((res) => {
                if (res && !res.success) {
                    ...
                    this.setState({ showScanner: false, isHashValid: false, isCertValid: false, serverResp: false, serverRespMsg: 'Hash Not Verified!' })
                }
            }).catch((err) => {
                console.log('err: ', err);
                alert(err);
        ...
        if (preState.dlHashOne !== this.state.dlHashOne || preState.dlHashTwo !== this.state.dlHashTwo) {
            if (this.state.dlHashOne !== null && this.state.dlHashTwo !== null) {
                if (this.state.dlHashOne === this.state.dlHashTwo) {
                    ...
                    this.setState({ isHashValid: false, serverResp: false, areHashesMatched: false, showScanner: false });
                }
            }
        }
    }
    ...
    _onPressVerifyCertButton = () => {
        this.connectingSoi(this.state.frCert)
            .then((resp) => {
                ...
                this.setState({ isCertValid: false, serverResp: false, serverRespMsg: JSON.parse(err.message) })
            })
    };
    ...
    _onPressButton(flag) {
        ...
        if (flag === 'sdl') {
            ...
            this.setState({ showScanner: true, qrMsg: 'Scanning First Responder QR Code' })
            ...
            this.setState({ isQrScanned: false, areHashesMatched: false, showScanner: true }, () => {
                this.scanner.reactivate();
            })
    ...
    render() {
        return (
            <View style={styles.outerContainer}>
                ...
                {!this.state.showScanner && this.state.isHashValid &&
                    <View style={styles.container}>
                        {this.state.isHashValid == true && !this.state.isCertValid ?
                            <View style={styles.badgeView}>
                                <BadgeIcon style={styles.sectionTitle} />
                                <TouchableOpacity>
                                    <Text style={styles.sectionTitle}>{this.state.serverRespMsg}</Text>
                                </TouchableOpacity>
                                <View style={styles.buttonContainer}>
                                    <TouchableOpacity
                                        onPress={() => this._onPressButton('sqr')}
                                        style={styles.button}
                                    >
                                        <Text style={styles.btnText}>Scan First Responder QR Code</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity
                            ...
                            : !this.state.isHashValid && !this.state.isCertValid &&
                            <View style={styles.badgeView}>
                                ...
                                <View style={styles.buttonContainer}>
                                    <TouchableOpacity
                                        onPress={() => this._onPressButton('try_again')}
                                        style={styles.button}
                                    >
                ...
                {!this.state.showScanner && this.state.isCertValid &&
                    <View style={styles.container}>
                        {this.state.isCertValid && !this.state.isHashValid ?
                            <View style={styles.badgeView}>
                                <BadgeIcon style={styles.sectionTitle} />
                                <TouchableOpacity>
                                    <Text style={styles.sectionTitle}>{this.state.serverRespMsg}</Text>
                            ...
                            <View style={styles.badgeView}>
                                ...
                                <View style={styles.bottomContainer}>
                                    <TouchableOpacity
                                        onPress={() => this._onPressButton('scan_again')}
                                        style={styles.button}
                                    >
                                        <Text style={styles.btnText}>Try Again</Text>
                                    </TouchableOpacity>
                                    {/* <Plus onPress={() => this._onPressButton('add')} fill={colors.blue} width={40} height={40} /> */}
                                </View>
                                <View style={styles.bottomContainer}>
                                    <TouchableOpacity
                                        onPress={() => this._onPressButton('cancel')}
